...
::: {.callout-tip collapse="true" title="Hint"}
You can create those 6 cards programmatically with `lapply()` and pass them with `!!!` inside `layout_columns_wrap()`.
:::
...
Tabs allow to organize related content over __multiple pages__ [^not_exactly]. They can be part of navbars (navigation), cards or standalone.
...
1. You'll switch to the cleaned-workshop branch, which has the 7 modules but without any existing UI nore server.
2. You may not need all modules depending on the wireframe.
3. TO DO
